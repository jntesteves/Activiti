	  ...
	  model.setUserTaskFormTypes(userTaskFormTypes);
		try {
          ...
          if (!model.getPools().isEmpty()) {
            activePool = model.getPools().get(model.getPools().size() - 1);
          }
          ...
          if (activePool != null) {
            extensionElementsParser.parse(xtr, activeSubProcessList, activePool, model);
          } else {
			...
			for (Process process : model.getProcesses()) {
			  for (Pool pool : model.getPools()) {
			    if (process.getId().equals(pool.getProcessRef())) {
			      ...
			      process.setResources(pool.getResources());
			    }
			  }
