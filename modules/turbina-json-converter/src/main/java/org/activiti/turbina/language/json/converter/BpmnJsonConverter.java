...
import org.activiti.turbina.constants.EditorJsonConstants;
import org.activiti.turbina.constants.StencilConstants;
import org.activiti.turbina.language.json.converter.util.JsonConverterUtil;
...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
  ...
  public BpmnModel convertToBpmnModel(JsonNode modelNode) {
    ...
    if (emptyPoolFound) {
      ...
      JsonNode processDocumentation = modelNode.get(EDITOR_SHAPE_PROPERTIES).get(PROPERTY_DOCUMENTATION);
      if (processDocumentation != null && StringUtils.isNotEmpty(processDocumentation.asText())) {
         process.setDocumentation(processDocumentation.asText());
      }
